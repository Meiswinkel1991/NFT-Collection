...
error CryptoDevs__ExceedMaximumSupply();
error CryptoDevs__NotEnoughEther();
error CryptoDevs__PresaleNotEnded();
error CryptoDevs__SendEtherFailed();
...
contract CryptoDevs is ERC721Enumerable, Ownable {
    ...
    function presaleMint() public payable onlyWhenNotPaused {
        ...
        if (tokenIds > maxTokenIds) {
            revert CryptoDevs__ExceedMaximumSupply();
        }
        if (msg.value < _price) {
    ...
    function mint() public payable onlyWhenNotPaused {
        if (block.timestamp > presaleEnded && !presaleStarted) {
    ...
    function withdraw() public payable onlyOwner {
        ...
        if (!sent) {
            revert CryptoDevs__SendEtherFailed();
        }
    }
